...
function analisarXml(texto, nome = 'XML colado') {
  ...
  const add = (nivel, etapa, mensagem) => r.verificacoes.push({ nivel, etapa, mensagem });
  ...
  if (original.getElementsByTagNameNS('*','parsererror').length) {
    add('erro','Sintaxe','XML malformado: ' + original.documentElement.textContent.trim().slice(0,1200)); return r;
  }
  ...
  for (const campo of campos) if (!inf.querySelector(campo)?.textContent.trim()) add('erro','Campos básicos',`Campo ausente ou vazio: ${campo}.`);
  if (!new RegExp(`^${tag}\\d{44}$`).test(inf.getAttribute('Id') || '')) add('erro','Chave',`Id deve começar com ${tag} e conter uma chave numérica de 44 dígitos.`);
  if (inf.querySelector('ide > mod')?.textContent.trim() !== (tag === 'NFe' ? '55' : '57')) add('erro','Identificação',`Modelo diferente do esperado para ${r.tipo}.`);
  ...
  if (tag === 'NFe') {
    ...
    inf.querySelectorAll('det').forEach((item, idx) => {
      ...
      if (!/^[1-9]\d*$/.test(numero || '') || numeros.has(numero)) add('erro','Produtos',`Item ${idx+1}: nItem ausente, inválido ou repetido.`);
      numeros.add(numero);
      for (const campo of ['xProd','qCom','vUnCom','vProd']) if (!item.querySelector('prod > ' + campo)?.textContent.trim()) add('erro','Produtos',`Item ${idx+1}: ${campo} ausente ou vazio.`);
    });
  }
...
function renderValidacaoXml() {
  ...
  document.getElementById('validacao-resultados').innerHTML = relatoriosXml.map((r, idx) => {
    ...
    return `<article class="painel-novo validacao-relatorio"><div class="painel-cabecalho"><h3>${escapeHtml(r.nome)}</h3><span class="validacao-selo">${escapeHtml(r.tipo)}</span></div>
      <p class="validacao-conclusao">${titulo}</p><ul class="validacao-lista">${r.verificacoes.map(v => `<li class="validacao-${v.nivel}"><strong>${{erro:'Erro',aviso:'Aviso',sucesso:'OK'}[v.nivel]} · ${escapeHtml(v.etapa)}</strong><span>${escapeHtml(v.mensagem)}</span></li>`).join('')}</ul>
